...
export default function IrrigationControl({ 
  ...
  // Duration for manual irrigation
    ...
    onMessage: (message) => {
      ...
      if (message.type === 'section_update' && 
        ...
        if (message.data.moisture_value !== undefined) {
          ...
          console.log('💧 Updating moisture value from', moisture, 'to', newMoistureValue);
        ...
        console.log('❌ Message not for this section or wrong type:', {
          ...
          farmIdMatch: parseInt(message.data?.farm_id) === farmId,
          sectionNumberMatch: parseInt(message.data?.section_number) === sectionNumber
        });
      }
    },
    onConnect: () => {
  ...
  const handleStartIrrigation = async () => {
    try {
      setLoading(true)
      console.log(`🚰 Starting irrigation for ${duration} seconds on Farm ${farmId}, Section ${sectionNumber}`);
  ...
  const handleStopIrrigation = async () => {
    try {
      setLoading(true)
      console.log(`🛑 Stopping irrigation on Farm ${farmId}, Section ${sectionNumber}`);
  ...
  const handleModeChange = async (newMode: string) => {
    try {
      setLoading(true)
      console.log(`⚙️ Setting mode to ${newMode} on Farm ${farmId}, Section ${sectionNumber}`);
  ...
  const handleThresholdUpdate = async () => {
    try {
      ...
      console.log(`🔧 Updating thresholds on Farm ${farmId}, Section ${sectionNumber}:`, { minThreshold, maxThreshold });
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        {mode === 'auto' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max="100"
                  value={minThreshold}
                  onChange={(e) => setMinThreshold(parseInt(e.target.value) || 30)}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  max="100"
                  value={maxThreshold}
                  onChange={(e) => setMaxThreshold(parseInt(e.target.value) || 70)}
                  disabled={thresholdLoading}
                />
              </div>
            </div>
            ...
            <div className="text-sm text-muted-foreground">
              <p>• Irrigation starts when moisture drops below {minThreshold}%</p>
              <p>• Irrigation stops when moisture reaches {maxThreshold}%</p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          <Label>Status</Label>
          <div className="flex items-center space-x-2">
            <div className={`w-3 h-3 rounded-full ${valveOpen ? 'bg-green-500' : 'bg-gray-300'}`} />
