...
export default function BulkOperations({
	...
	// Get section numbers for selected sections
	const getSelectedSectionNumbers = () => {
		return sections
	...
	const getOperationDescription = () => {
		switch (operation) {
			case "irrigate":
				return `Start irrigation for ${
					selectedSections.length
				} sections for ${Math.floor(duration / 60)}m ${duration % 60}s`;
			case "stop":
				return `Stop irrigation for ${selectedSections.length} sections`;
			case "mode":
				return `Set ${selectedSections.length} sections to ${mode} mode`;
			case "config":
				return `Update thresholds to ${minThreshold}%-${maxThreshold}% for ${selectedSections.length} sections`;
		}
	};
