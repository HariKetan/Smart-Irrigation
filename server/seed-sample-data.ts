...
async function seedSampleData() {
	...
	try {
		...
		if (farms.length === 0 || sections.length === 0) {
			console.log(
				"❌ No farms or sections found. Please run the farm seed script first."
			);
			return;
		}

		console.log(
			`📊 Found ${farms.length} farms and ${sections.length} sections`
		);
		...
		console.log(
			`📱 Inserting ${deviceStatuses.length} device status records...`
		);
