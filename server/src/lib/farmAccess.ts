 ...
 */
export const validateFarmAccess = (farmIdSource: 'query' | 'body' | 'params' = 'query') => {
    ...
    try {
      ...
      switch (farmIdSource) {
        ...
        case 'body':
          ...
          break;
        case 'params':
          farmId = parseInt(req.params.farm_id as string);
          break;
        default:
          farmId = parseInt(req.query.farm_id as string);
      }
      ...
      if (!farmId || isNaN(farmId)) {
        if (req.user?.farm_ids && req.user.farm_ids.length > 0) {
          ...
          return res.status(400).json({ error: 'No farm_id provided and user has no assigned farms' });
        }
      }
